...
export function Sidebar({ currentPage, onPageChange }: SidebarProps) {
  ...
  const handleNavClick = (id: string) => {
    if (!isExpanded) {
      setIsExpanded(true);
      setTimeout(() => onPageChange(id), 150);
    } else {
      onPageChange(id);
    }
  };
      ...
      >
        ...
        <div className="p-4 flex justify-center">
          <button
            ...
            className="w-12 h-12 bg-gradient-to-br from-sidebar-primary to-sidebar-primary/80 rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-all duration-300"
          >
            <span className="text-sidebar-primary-foreground font-semibold text-lg select-none">
              Y
            </span>
          </button>
        </div>
